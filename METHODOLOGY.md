...
Для семи профессий без прямого значения в общем отчёте добавлен снимок на 22 июля 2026 года из публичного SEO-описания отфильтрованной страницы калькулятора Хабр Карьеры: Data Scientist, MLOps, Computer Vision, специалист и инженер по информационной безопасности, SOC-аналитик и пентестер. Сохраняется только открыто показанная медиана. Расширенные распределения и грейды, скрытые входом в аккаунт, не извлекаются; размер выборки и gross/net у этих точек остаются `unknown`. Живая страница может измениться, поэтому число хранится как датированный снимок, а не автоматически обновляемый факт.
