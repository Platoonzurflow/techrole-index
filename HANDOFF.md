...
- после одобрения владельца запущен Tailscale Funnel `https://win-702hpohbtiv.tail044b19.ts.net` на `127.0.0.1:3199`; актуальный immutable `.next-public-tsnet-v017` собран с этим canonical. После публикации зарплатного и недельного контекста все постоянные сервисы healthy, внешний smoke подтвердил 50 профессий и 430 строк salary CSV, 742 daily slices, три provenance-слоя, 86 sitemap URL и условные ответы `304`. Платежи на preview используют только `demo/test`, terms остаются draft. Funnel остаётся beta и host-dependent preview, а не отказоустойчивым production;
