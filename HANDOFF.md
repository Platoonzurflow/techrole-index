...
- после одобрения владельца запущен Tailscale Funnel `https://win-702hpohbtiv.tail044b19.ts.net` на `127.0.0.1:3199`; актуальный immutable `.next-public-tsnet-v019` собран с этим canonical. После завершения зарплатного покрытия и уточнения условий личного ведения внешний smoke подтвердил один комплект Junior/Middle/Senior, 37 прямых / 13 смежных / 0 category-only профессий, 50 профессий и 432 строки salary CSV, 742 daily slices, три provenance-слоя, 86 sitemap URL и условные ответы `304`; `/mentorship` публикует индивидуальное согласование стоимости, все постоянные сервисы healthy. Платежи на preview используют только `demo/test`, terms остаются draft. Funnel остаётся beta и host-dependent preview, а не отказоустойчивым production;
