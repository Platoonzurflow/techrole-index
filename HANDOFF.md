...
- Production release `0d322afe` развёрнут на `techrole.ru`; пользовательские изменения `infra/Caddyfile` и `infra/Caddyfile.pre-faktvybor-20260727` на VDS сохранены. GitHub CI и Secret scan прошли для всех кодовых изменений релиза.
- Последний полный разрешённый HH-проход использовал 270 запросов и 743 страницы: 57 351 строка выдачи после 36 962 повторных появлений дала 20 389 уникальных вакансий текущего поискового снимка. Ошибок запросов нет; фактический период 2026-07-07..2026-08-06.
- Поиск теперь выполняется точной фразой только в поле названия вакансии и хранится отдельно от консервативной канонической классификации. Последний успешный run содержит 24 159 many-to-many связей с 50 профессиями; минимум — 8 у React Native developer. Для AI Engineer сохранено 180 уникальных вакансий: одиночный запрос `AI-инженер` дал ровно 72, `AI Engineer` — 70, остальные добавлены разрешёнными алиасами и дедуплицированы внутри роли.
- Все 20 389 уникальных вакансий query-match слоя обработаны detail-проходом: 20 388 карточек успешно получены, одна недоступна в HH API; необработанных карточек и ошибок обогащения нет. Во всём HH-хранилище сохранено 94 702 связи вакансий с навыками.
- Материализован новый query-match слой: 8 361 HH-срез и 24 159 profession-publication связей; 10 801 связь раскрывает зарплату, 4 768 входят в выборку midpoint. Полная замена снимка удалила 106 устаревших срезов, rolling rebuild создал 14 595 строк метрик по 50 профессиям.
- Все 50 профессий имеют собственные HH-наблюдения и не менее пяти поисковых совпадений. У каждой роли показываются собственные HH-счётчики; пустой неиспользуемый показатель направления удалён. Dashboard показывает пять ведущих компаний и агрегат «Другие компании».
- Индекс всех 50 профессий пересчитан по `v1.2.0`. Десять слабонаблюдаемых ролей заменены более массовыми, старые URL перенаправляются; новые роли и точные объёмы описаны в release `bea6dc9d`.
- Мобильная profession page стала почти вдвое короче: контрольный Android viewport 390×844 для «Аналитика данных» уменьшился с 15 908 до 8 224 CSS px без горизонтального overflow. Главный график имеет высоту 264 px, donut работодателей — 200 px; даты, легенды, tooltip, KPI, карточки и горизонтальные rails адаптированы для touch. Периоды 90/180 дней главного графика и остальные временные графики по-прежнему закрыты Premium на серверной выдаче. Карточки Junior/Middle/Senior повторяют те же последние значения, `n` и scope, что KPI главного графика: после финального полного снимка для Technical Support это 60/97/166 тыс. ₽; строка «Вилки „Работы России“» удалена.
- Финальный gate: Ruff/mypy/153 backend tests в CI, ESLint, TypeScript, 73 frontend tests, Next production build на 103 страницы, 46 Playwright passed + 1 intentionally skipped и public crawl 90/90. IndexNow принял 90 URL.
- Финальный production backup: `/opt/techrole-index/backups/hh-query-match-final-20260806T001733Z.sql.gz`, 18 052 154 bytes, mode `600`, gzip проверен; SHA-256 `6e2e3e41003fdfce958a218476d5000ceacf75578c71603329c323ae6cea3119` и sidecar также mode `600`.
...
Важно: текущая базовая gross-витрина детерминированно подготовлена и не должна выдаваться за полностью актуальное состояние рынка. 2026-07-19 выполнен полный разрешённый 180-дневный проход отдельного real-provenance контура официального API «Работа России»; nightly продолжает обновление. После безопасной повторной классификации `rules-v2` materialized dataset за 2026-01-21..2026-07-20 содержит 1 132 классифицированные публикации и 742 изолированных daily SQL-среза версии `observed-publications-v1`; invalid slices и salary leaks ниже `n=20` равны нулю. Rolling API на 2026-07-21 использует 180 полных UTC-дней 2026-01-23..2026-07-21: 5 646 записей источника, 1 278 классифицированных и 1 268 с раскрытой зарплатой. Накопительная materialized history сохраняет собственный более ранний cut-off. Неизвестный gross/net не смешивается с основной gross-витриной. С 2026-08-05 одобренный HH API добавлен отдельным 365-дневным поисковым снимком с собственным provenance, дедупликацией и раздельными gross/net-агрегатами; точная статистика последнего прохода приведена ниже.

Локальный полный HH-проход 2026-08-05 выполнил 197 запросов по названиям и алиасам, прочитал 1 406 официальных страниц/131 869 строк выдачи и после глобальной дедупликации сохранил 42 153 уникальные вакансии. `rules-v2` уверенно классифицировал 8 868 записей и покрыл все 50 профессий; 33 285 неоднозначных записей не были насильно распределены. В классифицированном слое 3 058 вакансий содержат зарплату: 1 056 gross, 2 002 net, unknown basis — 0; полных RUB gross-вилок 423, удалённых 2 618. Наблюдаемый период фактически 2026-07-06..2026-08-05, потому что API возвращает доступный поисковый снимок, а не обещанный полный годовой архив. Материализован 3 971 daily-срез с 8 868 публикациями и quality gate `n=3`. Одновременный catch-up Dagster и ручной запуск обнаружили отсутствие run-lock; теперь новый non-stale `running` run блокирует второй ingestion того же source.

Production-проход 2026-08-05 завершён без ошибок: 199 запросов, 1 410 страниц, 132 283 строки выдачи, 90 163 повторных появления и 42 120 уникальных вакансий. Классифицировано 8 870 записей по всем 50 профессиям; 33 250 неоднозначных записей сохранены без принудительной классификации. В публичном классифицированном слое 3 063 вакансии с зарплатой: 1 059 gross, 2 004 net, unknown basis — 0; 424 полные RUB gross-вилки и 2 615 удалённых вакансий. Материализовано 3 967 daily-срезов и 8 870 публикаций за фактически доступный период 2026-07-06..2026-08-05. Post-import backup: `/opt/techrole-index/backups/hh-import-20260805T095626Z.sql.gz` плюс SHA-256 sidecar; оба файла имеют mode `600`.

Финальное production-обновление HH 2026-08-05 прочитало 42 050 записей выдачи и довело точный классифицированный слой до 8 957 вакансий по всем 50 профессиям. Детальные карточки успешно получены для 8 938 вакансий, 19 недоступны в API, ошибок обогащения нет; извлечено 36 505 упоминаний ключевых навыков. У каждой профессии теперь собственное распределение работодателей; публичный dashboard показывает топ-5 и агрегат «Другие компании». Для 40 профессий доступно не менее пяти точных HH-вакансий. Версия scoring `v1.2.0` пересчитала оценки всех 50 профессий только из их собственных классифицированных HH-наблюдений: спрос, недельная динамика, доля вакансий без опыта, удалёнка, полнота данных и зарплата. Зарплатный компонент использует точную медиану полных RUB gross-вилок при `n≥5`; при меньшей выборке применяется нейтральная медиана среди пригодных профессий, а не данные направления. При общем `n<5` динамика также нейтральна, остальные имеющиеся точные признаки сохраняются; category fallback в HH-оценке и HH-графике отключён. Production deploy `9faa32075ddfbc1d2cdd71724dd2c0840c302ac6`, миграция `0011`, все постоянные Compose-сервисы healthy. IndexNow принял 90 URL. Финальный backup: `/opt/techrole-index/backups/hh-enriched-20260805T123111Z.sql.gz` плюс SHA-256 sidecar; gzip проверен, оба файла имеют mode `600`.

2026-07-21 добавлен третий независимый зарплатный слой. Последний отчёт Хабр Карьеры за I полугодие 2026 (`n=45 226`) даёт публичные P10/median/P90, технологии и региональные медианы; отдельные грейды дополнены публичными статьями Хабр Карьеры и Grades/GetGrade с сохранением `n` и tax status. 2026-07-22 из публичных SEO-описаний калькулятора Хабр Карьеры добавлены только открытые медианы: восемь точных ролей и два явно смежных ориентира — ML-разработчик для NLP, ERP-программист для SAP. Скрытые распределения не извлекались, `n` и gross/net помечены неизвестными. Общий опрос Профсоюза работников ИТ (`n=1 539`) закрывает только отсутствующие грейды явно подписанными медианами рынка: Junior 114 500 ₽, Middle 200 000 ₽, Senior 310 000 ₽. В итоге все 50 профессий имеют 150 из 150 грейдовых значений; основной ролевой срез составляет 37 прямых и 13 смежных, category-only ролей нет. На profession page они выводятся одним комплектом Junior/Middle/Senior: официальный midpoint публикуется с `n≥3`, но используется для всего комплекта только при полной непротиворечивой последовательности; иначе берётся единый наиболее точный исследовательский срез, а официальный `n` остаётся видимым. Прежние повторные карточки prepared/official слоёв удалены, а динамические графики сохранены отдельно. Опциональный Dagster-аудит проверяет только публичные метаданные, не обновляет числа автоматически и отличает drift от временной недоступности; итог каждого включённого запуска сохраняется в PostgreSQL и кратко показывается на `/status`. `/salary-benchmarks`, JSON и CSV публикуют весь слой с provenance и без Premium-полей, но не объявляют единую открытую лицензию вместо условий первичных источников. Значения доступны даже на teaser-страницах, не участвуют в score и не записываются в vacancy metrics.
...
2026-07-24 устранено системное противоречие графиков профессии. Подготовленный `vacancy_count` теперь везде называется расчётным объёмом витрины и не выдаётся за текущий остаток активных вакансий; дублирующий подготовленный зарплатный график удалён. Официальный API отдельно отдаёт общие и дневные счётчики полных RUB-вилок для точной профессии и направления. Страница агрегирует их по неделям в график полноты «Доля публикаций с полной зарплатной вилкой». Динамический зарплатный ряд применяет к midpoint явную отсечку от видимой базовой медианы: Junior 40%, Middle 70%, Senior 100%. Каждая дневная точка показывает среднее за предшествующие 30 дней при `n≥3`; для временного ряда нужны наблюдения с трёх разных дат, после чего scope выбирается как точная профессия, направление или общий IT-рынок. Статичный ориентир остаётся только последним fallback. Исходные публикации и открытые salary slices не изменяются, а metric/window/reference median/scope/коэффициенты/пороги публикуются в API. Карточки Junior/Middle/Senior выбирают один полный неубывающий комплект одного источника; 50 профессий имеют резервный полный комплект.

## 2. Где находится проект и что сейчас запущено

Рабочий каталог Windows:

`C:\Users\user\Documents\Codex\2026-07-21\techrole-index-c-users-user-documents\work\techrole-index`
...
Считаются vacancy count, salary count, coverage, median, average, P25, P75, медианы нижней/верхней границы, sample size и confidence. Midpoint создаётся только при двух границах. Gross/net не смешиваются. Минимум по умолчанию — 3 наблюдения; `n=3` считается полноценным публикуемым срезом и в UI называется базовой выборкой, а не ненадёжным результатом.
...
`DemoVacancyProvider` включён по умолчанию. Приложение TechRole Index №26065 одобрено HeadHunter для заявленной публичной коммерческой агрегированной аналитики. `HhApiProvider` включается только при одновременной настройке:

- `HH_ENABLED=true`;
- `HH_COMMERCIAL_USE_CONFIRMED=true`;
- `HH_CONTACT_EMAIL`;
- `HH_APP_NAME`;
- серверный `HH_ACCESS_TOKEN` либо пара `HH_CLIENT_ID` + `HH_CLIENT_SECRET`.

HhApiProvider использует только официальный API, до 100 записей на страницу и до 20 страниц/2 000 результатов на отдельный поисковый запрос, корректный HH-User-Agent и не обходит rate limit, CAPTCHA или глубину выдачи. При официальной ошибке `oauth/token_expired` провайдер один раз получает новый application token через `client_credentials` и повторяет исходный запрос; секрет остаётся только на сервере. Каждая роль и её алиасы ищутся точной фразой с `search_field=name`; vacancy id дедуплицируются внутри профессии в `vacancy_profession_matches`, а одна вакансия может входить в несколько честных ролевых срезов. Raw payload и detail enrichment ограничены allowlist без описаний, контактов, адресов, response URL и персональных данных; сохраняются навыки, языки, опыт, условия работы и публичные id/названия работодателей. Публично работодатели показаны только как топ-5 по профессии плюс «Другие компании»; gross/net остаются отдельными. HTML scraping, прокси-ротация и чужие логотипы запрещены.

Одобрение относится к описанному в заявке сценарию и не разрешает превращать API в безлимитную копию базы. Публично выдаются только агрегаты с оговоркой о поисковом снимке; application token и client credentials остаются только в `.env`/production secret environment.
...
- HH-приложение №26065 одобрено и интегрировано как source-isolated агрегированный поисковый снимок; полная историческая репликация, публикация raw-текстов и обход ограничения 2 000 результатов на запрос не выполняются;
...
4. Accessibility, lab performance budgets, Playwright 43/43 и внешний production crawl 88/88 проходят; остаются полевые p75 Core Web Vitals/RUM после накопления реального трафика.
...
4. HH вызывать только через одобренное приложение №26065, при всех guard-переменных и server-only token; не обходить лимиты и не публиковать raw-вакансии.
...
The comparison table now resolves each salary level through a transparent exact-profession → sourced-benchmark → category chain instead of reading only the last prepared-metric date. This order is intentional: a broad official category can be dominated by another role (the current Data & AI slice is mostly database-administrator vacancies) and must not outrank a sourced grade benchmark. Publication totals and remote-work share still use exact/category context, while their captions name «Работа России» and show the actual remote numerator and denominator. Category context returned with profession detail includes category salary slices, disclosed-salary count, remote count and confidence. Catalog category selection submits immediately, cards are defensively filtered by `category_slug`, and the search and direction controls were reordered. Pricing shows 1,349 RUB as the crossed-out original plan beside the current server price of 290 RUB; the old price uses a clean horizontal strike, and an already-owned plan card is muted.
...
## Autonomous completion handoff (2026-07-23)

- Account/dashboard/alerts/pricing now have distinct guest/free/Premium states, access expiry, useful shortcuts and owner-scoped payment/refund history without provider identifiers or sensitive values. Playwright covers guest accessibility/mobile overflow plus free→sandbox→Premium UI in dark mobile mode.
- Payments use an extensible server registry and migration `0008` stores an immutable SKU/receipt/fulfillment snapshot. Test/live Robokassa credentials are explicit and live never falls back. A separate `PAYMENTS_ROBOCHEKI_SMZ_CONFIRMED` gate keeps live fail closed. Refunds recompute stacked access. «Личное ведение» remains a non-payment application. Production must remain `robokassa/test` until the owner rechecks active Robocheki in both Robokassa and «Мой налог», then personally performs the 290 ₽ control payment.
...
- The payment and analytics release sequence is complete: CI, E2E, public audit and secret scan passed; production analytics is enabled with independent server-only keys; Robokassa remains enabled in the owner-verified live mode after the successful control payment.
...
- ФИО исполнителя рендерится только в реквизитах `/legal/offer`; политика обработки данных и согласия ссылаются на оферту и не дублируют ФИО. E2E-регрессия сравнивает страницы без вывода значения в отчёт.
- Отдельный график «Доля публикаций с полной зарплатной вилкой» удалён со всех 50 профессий; базовые поля открытого набора не удалялись. На узких экранах оглавление стало однострочным прокручиваемым, панели и KPI компактнее, а ключевые графики остаются внутри ширины экрана.

## Weekly research handoff (2026-07-25)

- В шапке появился публичный раздел «Исследования». `/insights` теперь сначала показывает датированные исследования, затем прежний корпус из 12 методических разборов.
- Первое исследование фиксирует полную неделю 18–24 июля 2026 года и сравнение с 11–17 июля: 71 против 137 классифицированных публикаций (−48,2%), 11 представленных профессий, 66 полных RUB-вилок и 78,9% наблюдений у трёх лидирующих ролей. Текст прямо запрещает переносить этот результат на весь рынок или текущий остаток вакансий.
- Статья хранит неизменяемый snapshot с периодом, временем обновления исходного daily dataset, KPI и таблицей профессий. Она публикуется как Schema.org `Report`, получает собственные CSL-JSON/BibTeX/RIS и автоматически входит в RSS, sitemap, `insights.json`, `ai-index.json`, Data Package и LLM-указатели.
- Финальный локальный gate: backend Ruff/mypy/141 pytest, frontend ESLint/TypeScript/69 Vitest, production build с 99 страницами, Compose config, 47 Playwright-сценариев и public crawl 89/89 — passed.

## HH salary and magical hero handoff (2026-08-08)

- Главная code-native сцена заменяет прежнего рисованного кандидата: Canvas-частицы собирают человеческий силуэт, SVG-лента связывает Junior/Middle/Senior с офером, а pointer/touch, обе темы, мобильная компоновка и `prefers-reduced-motion` проверены в production Playwright.
- Основной зарплатный график и три карточки уровня теперь используют только полные gross-вилки HH в RUB для query-match этой профессии. Порог публикации — `n≥5`; если уровня не хватает, исследовательский ориентир виден только как явно подписанный резерв. «Работа России» сохранена отдельным компактным слоем публикаций без конкурирующего salary-графика.
- Успешный HH snapshot №15 содержит 20 306 уникальных query-match вакансий; 20 305 доступны с безопасным detail-enrichment. Материализованы 24 056 публикаций, 14 873 дневных profession-metric строк по всем 50 ролям за фактически наблюдаемый период 2026-07-07—2026-08-07. История сохраняет реальные дни максимум до 180, но не дорисовывает даты до начала интеграции.
- Оценки `v1.2.0` пересчитаны для всех 50 профессий на 2026-08-07; текущий диапазон 19,5—68,3. Dagster теперь повторяет этот пересчёт после каждого полностью успешного HH materialization.
- Free API физически обрезает временные массивы до 30 дней, Premium может вернуть до 180 уже накопленных дней. Atomic snapshot guard не публикует частичную выдачу. Compact open-data catalog не строит невидимую salary history и кешируется в Redis; прогретый запрос снизился примерно с 6 секунд до 0,02 секунды.
- Production-like standalone slot `.next-public-tsnet-v023` развёрнут через Tailscale Funnel `https://win-702hpohbtiv.tail044b19.ts.net`; `public-frontend`, постоянный proxy, backend, worker, scheduler, Dagster, PostgreSQL и Redis healthy. Обновлённый внешний smoke проверяет 50 AI entities, 50 datasets, четыре provenance layer, 13 статей и их citation formats.
- Финальные gate: backend Ruff, mypy и 153 pytest; frontend ESLint, TypeScript, 73 Vitest и production build на 103 страницы; Compose config; 42 production Playwright сценария; полный внешний preview smoke — passed.
