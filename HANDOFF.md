...
- после одобрения владельца запущен Tailscale Funnel `https://win-702hpohbtiv.tail044b19.ts.net` на `127.0.0.1:3199`; актуальный immutable `.next-public-tsnet-v014` собран с этим canonical, все постоянные сервисы healthy, внешний smoke проверил 85 URL, 742 daily slices и три provenance-слоя. Платежи на preview используют только `demo/test`, terms остаются draft. Funnel остаётся beta и host-dependent preview, а не отказоустойчивым production;
