...
- после одобрения владельца запущен Tailscale Funnel `https://win-702hpohbtiv.tail044b19.ts.net` на `127.0.0.1:3199`; актуальный immutable `.next-public-tsnet-v018` собран с этим canonical. После объединения зарплатных карточек внешний smoke подтвердил ровно один комплект Junior/Middle/Senior на profession page, 50 профессий и 430 строк salary CSV, 742 daily slices, три provenance-слоя, 86 sitemap URL и условные ответы `304`; все постоянные сервисы healthy. Платежи на preview используют только `demo/test`, terms остаются draft. Funnel остаётся beta и host-dependent preview, а не отказоустойчивым production;
