...
2026-07-24 устранено системное противоречие графиков профессии. Подготовленный `vacancy_count` теперь везде называется расчётным объёмом витрины и не выдаётся за текущий остаток активных вакансий; дублирующий подготовленный зарплатный график удалён. Официальный API отдельно отдаёт общие и дневные счётчики полных RUB-вилок для точной профессии и направления. Страница агрегирует их по неделям в график полноты: публикации, полные вилки и доля пригодных записей. Единый 180-дневный зарплатный график сохраняет сплошные наблюдаемые линии без перестановки значений, дополняет отсутствующие уровни пунктирными статичными ориентирами одного непротиворечивого исследования и объясняет возможные пересечения Middle/Senior разным составом выборок. Карточки Junior/Middle/Senior выбирают один полный неубывающий комплект одного источника; 50 профессий имеют резервный полный комплект.
