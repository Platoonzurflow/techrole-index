...
2026-07-21 добавлен третий независимый зарплатный слой. Последний отчёт Хабр Карьеры за I полугодие 2026 (`n=45 226`) даёт публичные P10/median/P90, технологии и региональные медианы; отдельные грейды дополнены публичными статьями Хабр Карьеры и Grades/GetGrade с сохранением `n` и tax status. 2026-07-22 из публичных SEO-описаний точных страниц калькулятора Хабр Карьеры добавлены только открытые медианы ещё восьми ролей, включая ручного тестировщика; скрытые распределения не извлекались, `n` и gross/net помечены неизвестными. Все 50 профессий имеют категорийный и региональный fallback: 37 — прямой срез, 11 — явно смежный, 2 — только категорийный. Опциональный Dagster-аудит проверяет только публичные метаданные, не обновляет числа автоматически и отличает drift от временной недоступности; итог каждого включённого запуска сохраняется в PostgreSQL и кратко показывается на `/status`. Эти значения доступны даже на teaser-страницах, не участвуют в score, не записываются в vacancy metrics и не подменяют «Недостаточно данных» официального слоя.

«Работа России» - самостоятельная государственная open-data база, а не данные hh.ru. На момент проверки официальный API сообщал 538 722 вакансии во всей базе. После `rules-v2` представлены 36 из 50 профессий; для 14 узких профессий уверенных совпадений нет, и нули оставлены честными. API не даёт статуса на каждый исторический день, поэтому 180-дневный ряд означает публикации по `creation-date`, а не одновременно активные вакансии.
...
- после одобрения владельца запущен Tailscale Funnel `https://win-702hpohbtiv.tail044b19.ts.net` на `127.0.0.1:3199`; актуальный immutable `.next-public-tsnet-v015` собран с этим canonical, все постоянные сервисы healthy, внешний smoke проверил 85 URL, 742 daily slices и три provenance-слоя. Платежи на preview используют только `demo/test`, terms остаются draft. Funnel остаётся beta и host-dependent preview, а не отказоустойчивым production;
...
- Dagster `verify_public_salary_benchmarks` включён локально и фактически подтвердил 8 из 8 публичных SEO-медиан без drift/unavailable; три bounded retries отличают сетевую недоступность от изменения числа, автоматическая перезапись запрещена, а последняя проверка сохраняется в `audit_logs` и доступна кратким безопасным summary через `/status`;
