...
- Production release `f186d42e` развёрнут на `techrole.ru`; пользовательские изменения `infra/Caddyfile` и `infra/Caddyfile.pre-faktvybor-20260727` на VDS сохранены. GitHub CI и Secret scan прошли.
- Последний полный разрешённый HH-проход использовал 270 запросов и 2 351 страницу: 224 516 строк выдачи после 161 553 повторных появлений дали 62 963 записи прохода и 65 712 уникальных HH-вакансий в текущем хранилище. Ошибок запросов нет; фактический период 2026-07-06..2026-08-05.
- Точный классифицированный слой содержит 13 404 вакансии. Детали успешно получены для 13 364, ещё 40 карточек недоступны в HH API; необработанных карточек и ошибок обогащения нет. Сохранено 55 246 связей вакансий с навыками.
- Материализовано 5 685 HH-срезов и 13 404 публикации; 4 837 публикаций раскрывают зарплату, 2 125 входят в выборку midpoint. Exact rolling rebuild создал 13 447 строк метрик по 50 профессиям.
- Все 50 профессий имеют собственные HH-наблюдения и не менее пяти точных вакансий; минимум — 6 у React Native developer. У каждой роли минимум пять разных работодателей. Dashboard показывает пять ведущих компаний и агрегат «Другие компании».
...
- Финальный production backup: `/opt/techrole-index/backups/profession-mobile-release-20260805T221320Z.sql.gz`, 15 859 012 bytes, mode `600`, gzip проверен; SHA-256 `de88ed106036125408e99afa3f12d415ddbea76bd0f44b8224aab42b19cd459d` и sidecar также mode `600`.
