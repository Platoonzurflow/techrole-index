...
Безопасный локальный preview использует `PAYMENTS_PROVIDER=demo`, `PAYMENTS_MODE=test`, `PAYMENTS_LIVE_CONFIRMED=false` и `PAYMENTS_LEGAL_APPROVED=false`. Для Robokassa test shop применяются только `ROBOKASSA_TEST_PASSWORD1/2`; live использует только `ROBOKASSA_LIVE_PASSWORD1/2/3` и никогда не делает fallback к неоднозначным legacy aliases. Миграции `0008` (snapshot товара) и `0009` (privacy-first analytics) должны быть применены до запуска backend.

Production guard требует отдельные live credentials, явные `PAYMENTS_LIVE_CONFIRMED=true`, `PAYMENTS_LEGAL_APPROVED=true`, `PAYMENTS_STABLE_HTTPS_CONFIRMED=true` и owner-only `PAYMENTS_ROBOCHEKI_SMZ_CONFIRMED=true`, утверждённую версию оферты, статус продавца и схему чеков. Последний fiscal gate ставится только после проверки Robokassa и связанного приложения «Мой налог». Для подтверждённого НПД + Robokassa обязателен `PAYMENTS_FISCALIZATION_MODE=robokassa`. Robokassa live принимает только официальные payment/refund endpoints и отдельный Password3. Любое несовпадение завершает backend до приёма трафика.
...
Search/AI release checklist и действия владельца находятся в [SEARCH_AI.md](SEARCH_AI.md). Privacy-first dashboard включается отдельными `ANALYTICS_ENABLED=true` и двумя разными секретами минимум 32 символа; подробности в [ANALYTICS.md](ANALYTICS.md). Эти значения передаются backend и frontend только через server `.env` и не печатаются в deployment logs.
...
Production is served from Selectel VDS `94.102.88.123` at `https://techrole.ru`. Selectel DNS records are `A @ -> 94.102.88.123` and `CNAME www -> techrole.ru.`. Caddy provisions HTTPS automatically and redirects the legacy `sslip.io` address to the apex. Legal, seller, fiscalization and credential readiness are confirmed; the successful owner control payment on 2026-07-24 permits `PAYMENTS_MODE=live`. After any Compose recreation on the VDS always use both `compose.yaml` and `compose.production.yaml`; the base file alone does not contain the production commands and network layout.
