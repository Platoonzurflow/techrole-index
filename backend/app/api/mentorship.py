...
from app.schemas import MentorshipRequestCreate, MentorshipRequestOut
...
def require_mentorship_csrf(
    request: Request,
    csrf_cookie: str | None = Cookie(
        default=None, alias=settings.mentorship_csrf_cookie_name
    ),
) -> None:
...
@router.get("/csrf")
def mentorship_csrf(response: Response):
    token = new_csrf_token()
    response.set_cookie(
        settings.mentorship_csrf_cookie_name,
        token,
        httponly=False,
...
@router.post(
    ...
    status_code=202,
    dependencies=[Depends(require_mentorship_csrf)],
)
def create_mentorship_request(
    ...
    contact_hash = _hash_identifier(payload.contact.lower())
    try:
        limiter.check(ip_hash, contact_hash)
    except RateLimitExceeded as exc:
        raise HTTPException(
            status_code=429,
            detail="Слишком много заявок. Попробуйте отправить сообщение позже.",
        ) from exc
    ...
    try:
        deliver_mentorship_request.delay(mentorship_request.id)
    except Exception:
        logger.exception(
            "mentorship_delivery_enqueue_failed",
            extra={"mentorship_id": mentorship_request.id},
        )

    return MentorshipRequestOut(
        reference=mentorship_request.public_id,
        status="queued" if settings.support_email_enabled else "saved",
        email_sent=False,
        message=(
            "Заявка отправлена. Я свяжусь с вами по указанному контакту."
            if settings.support_email_enabled
            else "Заявка зарегистрирована. Сохраните её номер."
        ),
    )
