...
export async function GET(request: Request) {
  ...
  const body = JSON.stringify(buildObservedPublicationCsvw(siteUrl, dataset));
  const summary = summarizeObservedPublicationMetrics(dataset.records);
  return conditionalResponse(request, body, {
    headers: {
      "Access-Control-Allow-Origin": "*",
      "Cache-Control": "public, max-age=1800, stale-while-revalidate=86400",
      "Content-Disposition": "inline; filename=techrole-index-open-data-daily.csv-metadata.json",
      "Content-Language": "ru-RU",
      "Content-Type": `${csvwMediaType}; charset=utf-8`,
      "Link": `<${siteUrl}/open-data-daily.csv-metadata.json>; rel="canonical", <${siteUrl}/open-data-daily.csv>; rel="describes"; type="text/csv", <${csvwConformanceUrl}>; rel="profile", <${siteUrl}/citation>; rel="cite-as"`,
