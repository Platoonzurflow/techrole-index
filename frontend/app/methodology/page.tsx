...
const faq = [
  ...
  ["Как считается зарплатная динамика?", "Для каждой профессии net приводится к gross, односторонние границы оцениваются по типичной ширине полных вилок, а доли трёх сегментов определяются требованиями HH к опыту. Отсортированные зарплаты делятся в этих пропорциях; каждая дневная точка показывает среднее сегмента за предшествующие 30 дней."],
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_.8fr]">
        <section className="panel p-6">
          ...
          <div className="mt-5 grid gap-4 leading-7 text-muted">
            ...
            <p>Пропорции Junior, Middle и Senior не задаются одинаковыми для всех. Для каждой роли они рассчитываются из собственных требований HH к опыту: без опыта, 1–3 года, 3–6 и более 6 лет. Для малой роли применяется слабое сглаживание к общерыночному распределению 15/45/40 и минимум пять наблюдений на сегмент при общей выборке n≥15.</p>
            <p>Зарплаты сортируются и делятся на непрерывные нижний, средний и верхний сегменты в рассчитанных пропорциях. Это делает линии монотонными и использует всю зарплатную выборку, но не утверждает, что конкретный работодатель назвал вакансию Junior, Middle или Senior. Динамика — среднее сегмента в скользящем окне 30 дней; free получает до 30 дней истории, Premium — до 180 фактически накопленных.</p>
          </div>
        </section>
        <section className="panel p-6">
