...
const faq = [
  ...
  ["Что происходит, если вилок мало?", "Медиана вакансий считается минимум по трём полным вилкам. При меньшей выборке на странице показывается отдельно подписанный ориентир из открытого зарплатного исследования."],
  ["Как считается зарплатная динамика?", "Только для динамического графика midpoint вакансии должен быть не ниже 40% видимой базовой медианы для Junior, 70% для Middle и 100% для Senior. Каждая дневная точка показывает среднее за предшествующие 30 дней; сначала берётся точная профессия, затем направление и общий IT-рынок. Исходные публикации и показатели полноты не удаляются."],
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-[1fr_.8fr]">
        <section className="panel p-6">
          ...
          <div className="mt-5 grid gap-4 leading-7 text-muted">
            ...
            <p>Gross, net и неизвестный налоговый статус не смешиваются. Налоговый статус каждого зарплатного среза указывается отдельно. Валюты пересчитываются с сохранением исходной валюты, курса, даты и источника курса.</p>
            <p>Медиана вакансий публикуется минимум по трём полным вилкам: n=3 считается достаточным проверяемым срезом. При меньшем числе наблюдений показывается отдельно подписанный ориентир из открытого исследования.</p>
            <p>Динамический график применяет воспроизводимую нижнюю границу к midpoint полной вилки: 40% видимой базовой медианы для Junior, 70% для Middle и 100% для Senior. Базовая медиана выбирается в порядке точная профессия → технология → смежная роль → направление. Каждая дневная точка — среднее за предшествующие 30 дней при n≥3. Для ряда нужны наблюдения минимум с трёх дат; после фильтра проверяются точная профессия, направление и общий IT-рынок. Это правило не меняет зарплатные ориентиры, исходные публикации, их число или график полноты.</p>
          </div>
        </section>
        <section className="panel p-6">
