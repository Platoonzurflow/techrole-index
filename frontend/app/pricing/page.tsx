...
const fallbackCatalog: PaymentCatalog = {
  ...
  products: [{
    ...
    description: "Полный доступ к аналитике",
    amount: "290.00",
    currency: "RUB",
    access_days: 30,
    service_result: "30 дней доступа ко всем Premium-функциям TechRole Index",
    ...
    <div className="shell py-14">
      ...
      {hasPremium ? (
        <div className="mx-auto mt-8 flex max-w-4xl flex-wrap items-center justify-between gap-4 rounded-2xl border border-amber-500/35 bg-amber-400/10 p-5">
          <div className="flex items-center gap-3">
            ...
            <div><strong className="text-lg">Premium уже активен</strong><p className="mt-1 text-sm text-muted">Все платные возможности доступны. Продлить срок можно в личном кабинете.</p></div>
          </div>
      ...
      <div className="mx-auto mt-10 grid max-w-4xl gap-5 md:grid-cols-2">
        ...
        <article className={`panel premium-plan-card border-amber-400/40 p-7 ${hasPremium ? "premium-plan-card-active" : ""}`}>
          ...
          {product ? <p className="mt-4 rounded-xl bg-muted/5 p-3 text-sm leading-6 text-muted">Результат услуги: {product.service_result}. В чеке: {product.receipt.name}.</p> : null}
          ...
          <Link href={hasPremium ? "/account" : user ? "/account" : "/register"} className="button-primary mt-8 w-full">{hasPremium ? "Управлять доступом" : user ? "Перейти к подключению" : "Создать аккаунт"}</Link>
