...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const categories = await safeApi<Category[]>("/categories", []);
  const category = categories.find((item) => item.slug === slug);
  return { title: category?.name ?? "Категория", description: category?.description, alternates: { canonical: `/categories/${slug}` } };
}
...
export default async function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="shell py-12"><p className="eyebrow">Категория</p><h1 className="mt-3 text-4xl font-bold">{category?.name ?? slug}</h1><p className="mt-4 max-w-3xl text-lg text-muted">{category?.description}</p><div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{professions.map((item) => <ProfessionCard key={item.slug} profession={item} />)}</div></div>;
}
