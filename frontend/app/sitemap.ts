...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const lastModified = latestIngestion
    ...
    : status.last_metric_date
      ...
      : undefined;
  const staticRoutes = ["", "/professions", "/top", "/companies", "/answers", "/research", "/insights", "/reports/weekly", "/data-status", "/salary-benchmarks", "/open-data-daily", "/pricing", "/mentorship", "/support", "/methodology", "/glossary", "/sources", "/citation", "/about"];

  return [
    ...staticRoutes.map((path) => ({
      url: `${base}${path}`,
      changeFrequency: path === "/open-data-daily" ? "daily" as const : "weekly" as const,
      priority: path === "" ? 1 : ["/companies", "/open-data-daily", "/salary-benchmarks"].includes(path) ? 0.8 : 0.7,
      lastModified,
    })),
    ...categories.map((item) => ({
