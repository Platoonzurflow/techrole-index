...
export default async function ComparePage({ searchParams }: { searchParams: Promise<{ slugs?: string }> }) {
  ...
  const allowed = new Set(professions.map((item) => item.slug));
  const params = await searchParams;
  const initialSlugs = (params.slugs ?? "").split(",").map((slug) => slug.trim()).filter((slug) => allowed.has(slug)).slice(0, 3);
  return (
    <div className="shell py-12">
      ...
      <CompareTool professions={professions} initialSlugs={initialSlugs} />
    </div>
  );
}
