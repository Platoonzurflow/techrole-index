...
export async function GET() {
  ...
  return Response.json({
    ...
    resources: [
      ...
      { name: "hh-market-daily", path: `${siteUrl}/hh-market-daily.json`, format: "json", mediatype: "application/json", description: "Source-isolated daily-агрегаты одобренного HH API по профессии, seniority, региону и gross/net status; поисковый снимок, не полная база." },
      { name: "hh-market-daily-csv", path: `${siteUrl}/hh-market-daily.csv`, format: "csv", mediatype: "text/csv", description: "Плоский переносимый вариант daily HH-срезов без текстов вакансий и данных работодателей." },
      { name: "hh-market-enrichment", path: `${siteUrl}/hh-market-enrichment.json`, format: "json", mediatype: "application/json", description: "Безопасные агрегаты подробных карточек HH: топ-5 работодателей и другие компании, навыки, языки, опыт и условия работы." },
      { name: "hh-market-enrichment-csv", path: `${siteUrl}/hh-market-enrichment.csv`, format: "csv", mediatype: "text/csv", description: "Плоская long-form таблица тех же HH-фасетов по профессиям." },
