...
export default async function AnswersPage() {
  ...
  const schema = {
    ...
    mainEntity: {
      ...
      isBasedOn: summary.hh_top_professions.length ? ["https://trudvsem.ru/opendata/api", "https://api.hh.ru/openapi/redoc"] : "https://trudvsem.ru/opendata/api",
  ...
  return <div className="shell py-12 lg:py-16">
    ...
    <nav className="profession-toc mt-7 flex flex-wrap gap-2" aria-label="Разделы коротких ответов"><a href="#top-professions">Профессии</a>{summary.hh_top_professions.length ? <a href="#hh-top-professions">HH API</a> : null}<a href="#salary-by-level">Зарплаты</a>{summary.hh_salary_by_level.some((level) => level.roles.length) ? <a href="#hh-salary-by-level">HH gross</a> : null}<a href="#regions">Регионы</a><a href="#dynamics">Динамика</a><a href="#limitations">Ограничения</a></nav>

    <section id="top-professions" className="panel mt-10 scroll-mt-36 p-6 sm:p-8"><p className="eyebrow">Спрос</p><h2 className="mt-2 text-2xl font-bold">Какие профессии чаще встречались в публикациях?</h2><p className="mt-3 leading-7 text-muted">Топ по числу классифицированных публикаций «Работы России» за {summary.date_from ?? "—"} — {summary.date_to ?? "—"}.</p>{summary.top_professions.length ? <ol className="mt-6 grid gap-3">{summary.top_professions.map((item, index) => <li key={item.slug} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-line p-4"><span><span className="mr-3 font-mono text-muted">{index + 1}</span><Link className="font-semibold text-accent" href={`/professions/${item.slug}#official-open-data`}>{item.name}</Link></span><strong className="font-mono">{item.publications.toLocaleString("ru-RU")}</strong></li>)}</ol> : <p className="mt-6 text-muted">В подтверждённом слое пока нет публикаций для этого среза; ноль не подставляется.</p>}</section>

    {summary.hh_top_professions.length ? <section id="hh-top-professions" className="panel mt-8 scroll-mt-36 p-6 sm:p-8"><p className="eyebrow">Официальный HH API</p><h2 className="mt-2 text-2xl font-bold">Какие профессии чаще встречаются в снимке HH?</h2><p className="mt-3 leading-7 text-muted">Топ классифицированных вакансий за {summary.hh_date_from ?? "—"} — {summary.hh_date_to ?? "—"}. Это поисковый снимок с пределом 2 000 результатов на запрос, а не полная историческая база.</p><ol className="mt-6 grid gap-3">{summary.hh_top_professions.map((item, index) => <li key={item.slug} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-line p-4"><span><span className="mr-3 font-mono text-muted">{index + 1}</span><Link className="font-semibold text-accent" href={`/professions/${item.slug}#hh-market-data`}>{item.name}</Link></span><strong className="font-mono">{item.publications.toLocaleString("ru-RU")}</strong></li>)}</ol></section> : null}
    ...
    {summary.hh_salary_by_level.some((level) => level.roles.length) ? <section id="hh-salary-by-level" className="panel mt-8 scroll-mt-36 p-6 sm:p-8"><p className="eyebrow">HH gross-зарплаты</p><h2 className="mt-2 text-2xl font-bold">Какие gross-вилки HH прошли порог выборки?</h2><p className="mt-3 leading-7 text-muted">Медиана midpoint только полных RUB-вилок с явным gross. В снимке с зарплатой: {summary.hh_salary_disclosed_count.toLocaleString("ru-RU")}; gross: {summary.hh_salary_gross_count.toLocaleString("ru-RU")}; net: {summary.hh_salary_net_count.toLocaleString("ru-RU")}; статус не указан: {summary.hh_salary_tax_unknown_count.toLocaleString("ru-RU")}.</p><div className="mt-6 grid gap-5 lg:grid-cols-3">{summary.hh_salary_by_level.map((level) => <article key={level.seniority} className="rounded-xl border border-line p-5"><h3 className="text-xl font-bold">{levelLabels[level.seniority]}</h3>{level.roles.length ? <ul className="mt-4 grid gap-4">{level.roles.map((role) => <li key={role.slug}><Link className="font-semibold text-accent" href={`/professions/${role.slug}#hh-market-data`}>{role.name}</Link><span className="mt-1 block text-sm text-muted">{money(role.median)} gross · n={role.sample_size}</span></li>)}</ul> : <p className="mt-4 text-sm text-muted">Нет срезов, прошедших порог выборки.</p>}</article>)}</div></section> : null}

    <section id="regions" className="panel mt-8 scroll-mt-36 p-6 sm:p-8"><p className="eyebrow">География</p><h2 className="mt-2 text-2xl font-bold">В каких регионах больше наблюдаемых публикаций?</h2><p className="mt-3 leading-7 text-muted">Сумма ежедневных классифицированных публикаций по региональному полю источника за доступный материализованный период.</p>{summary.top_regions.length ? <div className="mt-6 table-wrap shadow-none"><table className="data-table"><thead><tr><th>Регион</th><th>Публикации</th></tr></thead><tbody>{summary.top_regions.map((region) => <tr key={`${region.code}:${region.name}`}><td>{region.name}</td><td className="font-mono">{region.publications.toLocaleString("ru-RU")}</td></tr>)}</tbody></table></div> : <p className="mt-6 text-muted">Региональный срез ещё не материализован; отсутствие данных не трактуется как ноль.</p>}</section>
