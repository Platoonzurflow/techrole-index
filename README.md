...
Форма личного ведения отправляется внутри сайта через отдельный endpoint `/api/v1/mentorship/requests`, сохраняет заявку с номером и передаёт её собственной Celery-задаче. Пользователь сразу видит, что может предложить комфортную стоимость в описании заявки, а формат, объём работы и итоговые условия согласуются индивидуально. Письма личного ведения имеют префикс `[TechRole Mentorship]`, а обращения поддержки - `[TechRole Support]`.
