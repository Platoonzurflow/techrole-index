...
Отдельный публичный зарплатный слой считает RUB midpoint только по двум границам, отдельно для Junior/Middle/Senior и при минимуме 3 полных вилок. История раз в неделю показывает накопительную медиану от начала 180-дневного периода и обязательно включает его последний день. Для каждого уровня точная профессия имеет приоритет; если она не образует временной ряд, график использует явно подписанный срез направления. Отдельный график полноты показывает по неделям новые публикации, полные RUB-вилки и их долю, поэтому поток публикаций больше не выглядит обещанием большой зарплатной выборки. Недостающие линии дополняются пунктирными статичными ориентирами, а наблюдаемые пересечения грейдов остаются неизменёнными и объясняются разным составом выборок. На странице профессии остаётся один непротиворечивый комплект грейдовых карточек из официального слоя или одного открытого исследования; это выбор по качеству источника, а не по наибольшей сумме. Фактический `n`, период, scope и налоговый статус сохраняются рядом со значением.
