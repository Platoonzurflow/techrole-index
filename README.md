...
Рабочий MVP русскоязычного сервиса аналитики рынка IT-профессий. Текущий публичный release — [`0.1.0`](CHANGELOG.md). Он показывает спрос, gross-зарплаты, Junior/Middle/Senior, выборку, confidence, 7/30/90-дневные тренды, типичный технологический стек и объяснимый индекс 0–100. Базовая витрина воспроизводимо подготовлена для 50 профессий, 4 геосрезов и 180 дней. Реальные записи официального открытого API «Работа России» показываются отдельным provenance-контуром: публикации и salary midpoint по уровням видны с размером выборки, но неизвестный gross/net не смешивается с основной gross-витриной. Отдельная инкрементальная SQL-витрина материализует только наблюдённые публикации по `creation-date` и не записывает их в подготовленные active-vacancy метрики.
