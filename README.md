...
После подтверждения владельцем статуса самозанятого НПД основной вариант — Robokassa с «Робочеками СМЗ», резервный — ЮKassa. Оба адаптера определяют товар и сумму на сервере. Robokassa передаёт подписанную номенклатуру HTML-формой `POST`, проверяет подпись ResultURL, дедуплицирует повторы и сверяет live-возвраты; ЮKassa использует REST idempotency и авторизованную перепроверку webhook. Магазин Robokassa прошёл KYC и активирован, но test/live credentials ещё не установлены на сервере, а «Робочеки СМЗ» должны отдельно получить статус «активен». До этого выполнены локальные и mock-контракты, а live-режим остаётся закрыт. Сравнение, безопасные переменные и инструкция: [PAYMENTS.md](PAYMENTS.md).
