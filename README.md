...
Рабочий MVP русскоязычного сервиса аналитики рынка IT-профессий. Текущий публичный release — [`0.1.0`](CHANGELOG.md). Он показывает спрос, зарплаты, Junior/Middle/Senior, выборку, confidence, 7/30/90-дневные тренды, типичный технологический стек и объяснимый индекс 0–100. Базовая gross-витрина воспроизводимо подготовлена для 50 профессий, 4 геосрезов и 180 дней. Реальные записи официального открытого API «Работа России» показываются отдельным provenance-контуром: публикации и salary midpoint по уровням видны с размером выборки, но неизвестный gross/net не смешивается с основной gross-витриной. Третий слой публикует фактические доходы из открытых исследований с явными scope, периодом, налоговым статусом и источником: последний срез Хабр Карьеры за I полугодие 2026 построен на 45 226 зарплатах и покрывает все 50 профессий прямым, смежным либо явно категорийным ориентиром.
...
Отдельный публичный зарплатный слой считает RUB midpoint только по двум границам, отдельно для Junior/Middle/Senior и при минимуме 20 полных вилок. История использует 30-дневные окна с недельным шагом. Если наблюдений меньше, UI и API возвращают «Недостаточно данных» вместо фиктивной зарплаты.

### Публичные ориентиры фактических доходов

`backend/app/data/salary_benchmarks.py` хранит версионированные факты из публичных отчётов отдельно от vacancy observations. Главный источник — [Хабр Карьера, I полугодие 2026](https://habr.com/ru/specials/1060148/), `n=45 226`: ролевые P10/median/P90, медианы технологий и значения Москва/Санкт-Петербург/другие регионы. Для грейдов, которых нет в общем отчёте, используются отдельно помеченные публикации [Хабр Карьеры о Junior](https://habr.com/ru/companies/habr_career/articles/1040188/) и [Grades/GetGrade 2025](https://habr.com/ru/articles/981704/) с сохранением малых `n` и `gross/net=unknown` у последнего. Дополнительно сохранён датированный снимок семи точных ролевых медиан, которые официальный [калькулятор Хабр Карьеры](https://career.habr.com/salaries) публично сообщает в SEO-описании отфильтрованных страниц; скрытые входом значения не извлекались, а `n` и gross/net остаются неизвестными.

Порядок интерпретации строгий: точная профессия → технологический или смежный срез → категорийный fallback. P10/P90 не выдаются за Junior/Senior, категорийная медиана не становится ролевой, а исследование фактических доходов не записывается в таблицы вилок вакансий. У всех 50 профессий есть Россия и три геосреза категории; 36 получили прямой срез, 12 — явно смежный, только 2 — категорийный ориентир.
