...
Отдельный публичный зарплатный слой считает RUB midpoint только по двум границам, отдельно для Junior/Middle/Senior и при минимуме 20 полных вилок. История использует 30-дневные окна с недельным шагом. Если наблюдений меньше, интерфейс сохраняет фактический `n`, но вместо пустой карточки показывает отдельный открытый исследовательский ориентир и его ограничения.
