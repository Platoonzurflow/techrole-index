...
Отдельный публичный зарплатный слой считает RUB midpoint только по двум границам, отдельно для Junior/Middle/Senior и при минимуме 3 полных вилок. Для динамического графика действует явная нижняя отсечка относительно видимой базовой медианы профессии/технологии/смежной роли/направления: 40% для Junior, 70% для Middle и 100% для Senior. Каждая дневная точка истории показывает среднее прошедших отсечку вилок за предшествующее 30-дневное окно. Для наблюдаемого временного ряда нужны подходящие публикации минимум с трёх разных дат; охват выбирается в порядке точная профессия → направление → весь IT-рынок и всегда подписывается. Если ни один слой не образует ряд, остаётся пунктирный статичный ориентир исследования. Исходные публикации, их число, открытые salary slices и график полноты не фильтруются. На странице профессии остаётся один непротиворечивый комплект грейдовых карточек из официального слоя или одного открытого исследования; это выбор по качеству источника, а не по наибольшей сумме. Фактический `n` окна, период, scope, пороги и налоговый статус сохраняются рядом со значением или в публичном JSON.
