...
Рабочий MVP русскоязычного сервиса аналитики рынка IT-профессий. Текущий публичный release — [`0.1.0`](CHANGELOG.md). Он показывает спрос, зарплаты, Junior/Middle/Senior, выборку, confidence, тренды, типичный стек и объяснимый индекс 0–100. Главный зарплатный график и грейдовые карточки используют только собственные совпадения профессии из официального HH API: полные gross-вилки в RUB, скользящее 30-дневное среднее и минимум пять наблюдений на точку. Бесплатный API и SSR получают не более 30 дней истории; Premium может запросить до 180 накопленных календарных дней, но интерфейс не дорисовывает период, которого ещё нет в наблюдениях. «Работа России», подготовленная 180-дневная витрина и открытые исследования остаются отдельными слоями: первый показывает provenance публикаций, второй — расчётный ряд спроса, а исследования служат явно подписанным резервом для редкого грейда. Индекс `v1.2.0` также использует только собственные HH-наблюдения каждой профессии; дефицит точных вилок получает нейтральный peer-ориентир и честно сниженное качество данных.

> Приложение TechRole Index №26065 одобрено HeadHunter для заявленного сценария публичной коммерческой агрегированной аналитики. Серверный токен хранится только в окружении; публично выводятся агрегаты, а не копия вакансий. Условия и технические ограничения зафиксированы в [`DATA_SOURCES.md`](DATA_SOURCES.md).
...
Команда не перезаписывает AI- или ручные решения. Текущий `rules-v3` объединяет слишком узкие технологические дубли с их устойчивыми профессиями и добавляет массовые роли с консервативными исключениями. Каноническая title-классификация остаётся консервативной, а полнота поиска HH хранится отдельно: точная фраза ищется только в названии вакансии, результаты алиасов дедуплицируются внутри профессии и могут образовывать many-to-many связь. Production-снимок от 2026-08-06 содержит 20 389 уникальных вакансий текущей выдачи и 24 159 связей с 50 профессиями; минимум отдельной роли — 8. Актуальные production-счётчики фиксируются в `HANDOFF.md` после каждого полного прохода.
...
3. Для одобренного приложения официального HH API установить серверный application token и все guards:
...
HH_USE_ALIAS_QUERIES=true
HH_DETAIL_REQUEST_DELAY_SECONDS=0.25
HH_DETAIL_MAX_RECORDS_PER_RUN=10000
HH_DETAIL_BATCH_SIZE=50
```

Provider использует официальный API, `HH-User-Agent`, страницы до 100 элементов и документированный предел глубины 2 000 результатов на один поисковый запрос. Запросы по имени и алиасам дедуплицируются по vacancy id; сохраняется только безопасный allowlist полей. Для уверенно классифицированных вакансий возобновляемый detail-проход получает навыки, языки, опыт, занятость, рабочие форматы, графики и публичные id/названия работодателей. Описания, контакты, адреса, ссылки отклика и персональные данные не сохраняются. На сайте работодатели публикуются только как топ-5 по профессии и единая группа «Другие компании». Источник изолирован от «Работы России», gross/net не смешиваются. HTML scraping, обход rate limit, CAPTCHA и прокси-ротация не используются. Условия: [DATA_SOURCES.md](DATA_SOURCES.md).

Ручной полный или ограниченный detail-проход:

```powershell
docker compose exec -T backend python -m app.enrich_hh_vacancies --max-records 10000 --batch-size 50
```

Команда идемпотентна, пропускает уже обработанные карточки и фиксирует прогресс пакетами. Ночной Dagster pipeline запускает её после успешного HH-поиска, поэтому в обычном режиме дозагружаются только новые вакансии.
...
Основной публичный зарплатный слой считает midpoint только по двум границам HH-вилки, только при `currency=RUR`, `gross=true` и отдельно для Junior/Middle/Senior. Каждая точка главного графика — среднее за предшествующее 30-дневное окно при `n≥5`; направление и общий IT-рынок в профессию не подставляются. Карточки грейдов повторяют последнюю доступную точку того же ряда и тот же `n`. Если точного ряда для уровня нет, остаётся явно подписанный статичный ориентир открытого исследования. Бесплатная выдача физически обрезает историю до 30 дней, Premium — до 180 накопленных дней; 90/180 не являются прогнозом и не раскрываются одной только кнопкой в браузере. «Работа России» продолжает публиковать свои точные счётчики и provenance отдельно и не участвует в главной зарплатной визуализации.
