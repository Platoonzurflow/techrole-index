...
После подтверждения владельцем статуса самозанятого НПД основной вариант — Robokassa с «Робочеками СМЗ», резервный — ЮKassa. Оба адаптера определяют товар и сумму на сервере. Robokassa передаёт подписанную номенклатуру HTML-формой `POST` на канонический `Merchant/Index.aspx`, проверяет подпись ResultURL, дедуплицирует повторы и сверяет live-возвраты; ЮKassa использует REST idempotency и авторизованную перепроверку webhook. Магазин Robokassa прошёл KYC и активирован, «Робочеки СМЗ» имеют статус «Активно», а тестовые и боевые credentials сохранены только в production `.env`. До завершения официального test checkout и отдельного live-переключения реальные списания остаются закрыты. Сравнение, безопасные переменные и инструкция: [PAYMENTS.md](PAYMENTS.md).
...
Отдельный публичный зарплатный слой считает RUB midpoint только по двум границам, отдельно для Junior/Middle/Senior и при минимуме 3 полных вилок. История раз в неделю показывает накопительную медиану от начала 180-дневного периода и обязательно включает его последний день. Для каждого уровня точная профессия имеет приоритет; если она не образует временной ряд, график использует явно подписанный срез направления. На странице профессии остаётся один комплект грейдовых карточек: официальный ролевой midpoint с достаточной выборкой имеет приоритет, иначе выбирается наиболее точный открытый исследовательский ориентир; это выбор по качеству источника, а не по наибольшей сумме. Фактический `n`, период, scope и налоговый статус сохраняются рядом со значением.
